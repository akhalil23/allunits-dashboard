...
import { PILLAR_LABELS, STATUS_COLORS, QUALIFIER_COLORS } from '@/lib/constants';
...
export default function SnapshotCard({ label, accent, items, config, observedAt }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    return { applicable: applicable.length, total: items.length, riskIndex, timeProgress, qualDist, pillarStats };
  }, [items, viewType, term, academicYear, observedAt]);

  const riskColor = stats.riskIndex >= 2 ? QUALIFIER_COLORS['Critical Risk'] : stats.riskIndex >= 1.2 ? QUALIFIER_COLORS['Emerging Risk'] : QUALIFIER_COLORS['On Track'];
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="card-elevated p-5 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="text-center p-3 rounded-lg bg-muted/50">
          <p className="text-2xl font-display font-bold text-primary">{stats.timeProgress.toFixed(0)}%</p>
          <p className="text-[10px] text-muted-foreground">Time Progress</p>
        </div>
      </div>

      {/* Qualifier bar */}
      <div>
        <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Qualifier Distribution</span>
        <div className="h-4 rounded-full overflow-hidden flex bg-muted mt-1">
          {stats.qualDist.filter(q => q.count > 0).map(q => (
            <div key={q.qualifier} className="h-full" style={{ backgroundColor: q.color, width: `${q.percent}%` }} title={`${q.qualifier}: ${q.percent}%`} />
          ))}
        </div>
        <div className="flex flex-wrap gap-2 mt-1.5">
          {stats.qualDist.filter(q => q.count > 0).map(q => (
            <div key={q.qualifier} className="flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: q.color }} />
              <span className="text-[9px] text-muted-foreground">{q.qualifier} {q.count}</span>
            </div>
          ))}
        </div>
      </div>
