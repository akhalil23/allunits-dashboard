      ...
      <section>
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="relative rounded-2xl border border-amber-500/40 bg-amber-500/5 shadow-sm overflow-hidden p-5 sm:p-6">
          ...
          <div className="space-y-3 text-sm text-muted-foreground leading-relaxed">
            <p>The dashboard operates on <span className="font-semibold text-foreground">controlled automated monthly reporting snapshots</span>. Source data is fetched, validated, and atomically published once per cycle — never queried live.</p>
            <p>Refreshes are <span className="font-semibold text-foreground">automatically scheduled on the 1st of each month at 02:00 UTC</span>. If a refresh fails, the previous validated monthly snapshot remains active and the system retries automatically.</p>
            <p>The timestamp shown across every tab reflects the <span className="font-semibold text-foreground">last successful validated monthly refresh</span>. There is no manual refresh button.</p>
          </div>
        </motion.div>
      </section>
