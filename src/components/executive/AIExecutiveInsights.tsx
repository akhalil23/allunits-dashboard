...
export default function AIExecutiveInsights({ aggregation }: Props) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-[10px] text-muted-foreground italic px-1">AI insights are generated from aggregated computed metrics only — no raw spreadsheet data is sent to the AI model.</p>

      {!hasGenerated && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="card-elevated p-8 sm:p-12 text-center space-y-4">
      ...
      {isLoading && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card-elevated p-8 text-center space-y-3">
      ...
      {error && hasGenerated && !isLoading && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card-elevated p-6 border-destructive/30 bg-destructive/5">
      ...
      {insights && !isLoading && (
        <AnimatePresence mode="wait">
          <motion.div key="results" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="space-y-4">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="card-elevated p-4 sm:p-6 relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-primary/[0.04] to-transparent pointer-events-none" />
              <div className="relative flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  <Brain className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">Executive Summary</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {insights.insights.map((insight, idx) => {
                ...
                return (
                  <motion.div key={idx} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * (idx + 1) }} className="card-elevated p-4 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full rounded-r" style={{ backgroundColor: color }} />
                    <div className="pl-3">
                      <div className="flex items-center gap-2 mb-2"><Icon className="w-3.5 h-3.5" style={{ color }} /><span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color }}>{insight.type}</span></div>
            ...
            {insights.recommendation && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="card-elevated p-4 sm:p-5 border-primary/20 bg-primary/[0.03]">
                <div className="flex items-start gap-3">
                  <Sparkles className="w-4 h-4 text-primary shrink-0 mt-0.5" />
                  <div>
                    <span className="text-[10px] text-primary font-semibold uppercase tracking-wider">Recommendation</span>
                    <p className="text-xs text-foreground mt-1 leading-relaxed">{insights.recommendation}</p>
                  </div>
                </div>
              </motion.div>
            )}

            <p className="text-[9px] text-muted-foreground/60 text-center px-4">AI-generated insights are based on aggregated metrics and should be validated by domain experts before decision-making.</p>
          </motion.div>
        </AnimatePresence>
      )}
    </div>
