...
import type { ActionItem, ViewType, AcademicYear } from '@/lib/types';
...
export default function IntelligencePanel({ items, viewType, observedAt, academicYear }: Props) {
  const timeProgress = useMemo(() => computeTimeProgress(observedAt, academicYear), [observedAt, academicYear]);
  const qualDist = useMemo(
    () => computeQualifierDistribution(items, viewType, observedAt, academicYear),
    [items, viewType, observedAt, academicYear]
  );
  const riskIndex = useMemo(
    () => computeRiskIndex(items, viewType, observedAt, academicYear),
    [items, viewType, observedAt, academicYear]
  );
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-4 px-1">
        ...
        <h3 className="font-display text-sm font-semibold text-muted-foreground uppercase tracking-wider">
          Intelligence Layer — Risk Signals
        </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card-elevated p-5">
          ...
          <p className="text-[10px] text-muted-foreground mt-2">
            AY {academicYear} • Based on server timestamp
          </p>
        </div>
        ...
        <div className={`card-elevated p-5 ${riskBg}`}>
          ...
          <p className={`text-4xl font-display font-bold ${riskColor}`}>
            {riskIndex.toFixed(2)}
          </p>
          ...
          <div className="mt-3 h-2 rounded-full bg-muted overflow-hidden">
            <motion.div
              ...
              style={{ backgroundColor: riskIndex >= 2 ? QUALIFIER_COLORS['Critical Risk'] : riskIndex >= 1.2 ? QUALIFIER_COLORS['Emerging Risk'] : QUALIFIER_COLORS['On Track'] }}
              initial={{ width: 0 }}
              animate={{ width: `${(riskIndex / 3) * 100}%` }}
              transition={{ duration: 0.8, ease: 'easeOut' }}
            />
          </div>
        </div>
      ...
      {/* Horizontal stacked qualifier bar */}
      <div className="card-elevated p-5 mt-4">
        <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider block mb-3">
          Qualifier Distribution (Applicable Items)
        </span>
